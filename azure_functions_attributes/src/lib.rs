...
mod inputs;
use inputs::{TimerTriggerInputs, QueueTriggerInputs, Binding};
...
fn impl_trigger<A>(args: TokenStream, item: TokenStream, trigger_type: &str) -> TokenStream where A: Binding + FromMeta {
    ...
    let outer_function = quote! {
        ...
        #vis async fn #function_ident((req, body): (actix_web::HttpRequest, actix_web::web::Json<azure_functions_types::#trigger_type>)) -> actix_web::Result<actix_web::HttpResponse> {
            //#(#definitions;)*
...
pub fn queue_trigger(args: TokenStream, item: TokenStream) -> TokenStream {
    let output = impl_trigger::<QueueTriggerInputs>(args, item, "QueueTrigger");
